...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    formRegister.addEventListener('submit', function(e) {
      ...
      let isValid = required(account, 'error-account') && validLength(account, 'error-account', 6, 25)
      isValid &= required(phoneNumber, 'error-phoneNumber') && validPhoneNunber(phoneNumber, 'error-phoneNumber')
      isValid &= required(fullName, 'error-fullName')
      isValid &= required(password, 'error-password') && validLength(password, 'error-password', 6, 25)
      isValid &= required(rePassword, 'error-rePassword') && validCheckRePassword(password, rePassword, 'error-rePassword')
      if(isValid) {
